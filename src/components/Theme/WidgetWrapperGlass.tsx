...
import { RotateCcw, X, TrendingUp, TrendingDown, Minus, Clock } from 'lucide-react';
  ...
  return (
    <Card className="bg-gradient-to-br from-blue-50/80 via-white/90 to-indigo-50/80 dark:from-slate-800/90 dark:via-slate-700/90 dark:to-blue-900/90 backdrop-blur-md border border-blue-200/50 dark:border-slate-600/50 transition-all duration-500 rounded-2xl overflow-hidden">
      ...
      <div className="relative px-5 pt-4">
        ...
        <div className="relative flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mb-2">
          <div className="flex-1">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              {/* Moved timestamp + refresh pill to the top */}
              <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
                <Clock className="h-3.5 w-3.5" />
                <span>Updated {new Date(widget.lastUpdated).toLocaleTimeString()}</span>
                <span className="px-2 py-0.5 rounded-full bg-white/60 dark:bg-slate-800/60 border border-white/40 dark:border-white/10">Every {widget.refreshInterval}s</span>
              </div>
            </div>
          </div>
      ...
      {/* Footer removed to make room at top; keep a subtle divider */}
