...
import { WidgetWrapperMinimal, WidgetWrapperGlass, WidgetWrapper } from '@/ui/theme';
...
export function TableWidget({ widget }: { widget: Widget }) {
  ...
  const renderContent = () => (
    <>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            ...
            className="pl-10 bg-white/60 dark:bg-slate-900/40 border border-border/60 backdrop-blur-md focus:bg-white dark:focus:bg-slate-900"
          />
        </div>
        
        <div className="flex items-center gap-2">
          <Select value={pageSize.toString()} onValueChange={(v) => setPageSize(Number(v))}>
            <SelectTrigger className="w-32 bg-white/60 dark:bg-slate-900/40 border border-border/60 backdrop-blur-md">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      {/* Data Limit Warning */}
      {isLimitedData && (
        <div className="mb-4 p-3 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/10 dark:to-orange-900/10 border border-amber-200/80 dark:border-amber-800/40 rounded-xl">
        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/50 dark:border-white/10 bg-white/60 dark:bg-slate-900/40 backdrop-blur-md">
                    ...
                    return (
                      <th key={field} className="text-left p-4 font-medium">
                        <Button
                          ...
                          className="font-medium text-xs text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-white/50 dark:hover:bg-slate-800/50 -ml-2 h-8 rounded-lg"
                        >
              ...
              <tbody>
                {pageData.map((row, index) => (
                  <tr 
                    ...
                    className="border-b border-white/40 dark:border-white/10 hover:bg-white/50 dark:hover:bg-slate-800/40 transition-colors backdrop-blur-sm"
                  >
          ...
          {shouldShowPagination && (
            <div className="px-5 py-4 border-t border-white/40 dark:border-white/10 bg-white/60 dark:bg-slate-900/40 backdrop-blur-md rounded-b-xl">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="text-sm text-slate-600 dark:text-slate-400">
                  Showing <span className="font-medium text-slate-900 dark:text-white">{startIndex + 1}</span> to{' '}
                  <span className="font-medium text-slate-900 dark:text-white">{endIndex}</span>{' '}
                  of <span className="font-medium text-slate-900 dark:text-white">{processedData.length}</span> results
                  {isLimitedData && (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    disabled={page === 1}
                    className="bg-white/70 dark:bg-slate-900/40 border border-border/60 backdrop-blur-md"
                  >
                      ...
                      Array.from({ length: totalPages }, (_, i) => i + 1).map(pageNum => (
                        <Button
                          ...
                          className={page === pageNum 
                            ...
                            : "bg-white/70 dark:bg-slate-900/40 border border-border/60 backdrop-blur-md"
                          }
                        >
                          {pageNum}
                        </Button>
                  ...
                  <div className="sm:hidden text-sm text-slate-600 dark:text-slate-400 bg-white/70 dark:bg-slate-900/40 px-3 py-1 rounded border border-border/60 backdrop-blur-md">
                    {page} / {totalPages}
                  </div>
  ...
  // Standardize: use the same shape/style for all widgets
  return (
    <div className="col-span-full">
      <WidgetWrapperGlass widget={{ ...widget, theme: 'glass' }}>
        {renderContent()}
      </WidgetWrapperGlass>
    </div>
  );
